 ...
 * 
 */
public class UUID64 implements IUUID<Long>{
	/** 有效的位数 */
	...
	/**
	 * 
	 * 构建一个系统默认的UUID64实例,5位rid(大区的ID),10位sid(服的ID),6位lid(线的ID),38位oid,在oid之前保留4位
	 * 
	 * @param rid
	 *            大区的ID
	 * @param sid
	 ...
	 * @return
	 */
	public static long[] splitUUIDByDefaultRule(final long uuid){
		long[] _splitIds = new long[4];
		_splitIds[0] = ((getMaskBits(5, MAX_BITS - 5)) & uuid) >> (MAX_BITS - 5);
		_splitIds[1] = ((getMaskBits(10, MAX_BITS - 15)) & uuid) >> (MAX_BITS - 15);
		_splitIds[2] = ((getMaskBits(6, MAX_BITS - 21)) & uuid) >> (MAX_BITS - 21);
		_splitIds[3] = (getMaskBits(38, 0)) & uuid;
		return _splitIds;
	}
